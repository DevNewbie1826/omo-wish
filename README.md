...
5. **동시 실행·검토** — 독립 태스크 DAG는 병렬 실행하고, 마지막 노드는 ultrabrain 검토(verdict: APPROVED/REVISE). REVISE면 같은 worktree에서 요구 사항으로 다음 mass ulw DAG를 만들고, 독립 변경은 deep agent를 병렬 디스패치한 뒤 합쳐 검증하고 ultrabrain이 재검토한다. APPROVED 전에는 merge하지 않는다
